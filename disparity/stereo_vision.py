...
BLOCK_SIZE = 7
SEARCH_BLOCK_SIZE = 28
FIX_WIDTH = 450

def _read_left_right_image_pair(left_image_path="fake/SomSirFake1.png",
                                right_image_path="fake/SomSirFake2.png"):
    ...
    left_im = resize(left_im)
    right_im = resize(right_im)
...
def resize(img): 
    ...
    if (width > FIX_WIDTH):
        height = int(FIX_WIDTH * int(img.shape[0]) / width)
        dim = (FIX_WIDTH, height)
